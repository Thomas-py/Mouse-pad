...
- **Estado**: hecho. `main.py` conecta de verdad `ControlServer` + `motion_server` + `injector` + `DiscoveryAnnouncer` + `TokenStore` (antes el CLI solo anunciaba mDNS y no arrancaba nada más — bug real encontrado y corregido acá). `protocol.py` ahora valida tamaño y rango de `count` en `decode_datagram` y atrapa `UnicodeDecodeError` en `decode_message`. `test_robustness.py` es el test end-to-end del "Hecho cuando": `ControlServer`+`motion_server` reales en loopback, flood de basura variada en ambos puertos (JSON roto, binario random, magic/versión corruptos, mensajes oversize, 60 conexiones TCP concurrentes) y después confirma pairing+handshake normal. No corre literalmente 60s de basura (impracticable en una suite de tests) — decisión propia documentada en el docstring del test: volumen y variedad en ~1s en vez de tiempo de pared, mismo criterio de fondo. 104 tests, todos verdes.
