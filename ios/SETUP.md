...
`RemotePadTests` (desde S-11) corre con `xcodebuild test -destination "platform=iOS Simulator,name=iPhone 17"`. El nombre exacto del simulador disponible cambia con el tiempo (Apple/Codemagic/GitHub actualizan las imágenes — se ajustó una vez ya, de `iPhone 15` a `iPhone 17` cuando el runner pasó a Xcode 26.6). Si el workflow falla por "no such simulator device" o "Unable to find a device matching...", correr `xcrun simctl list devices available` en el runner (o un paso de debug) y actualizar el nombre en `codemagic.yaml` y `.github/workflows/ios-build.yml`.
